...
6. 上传首部块，第 0 块完成后尽早检查 frame 摘要。解出 manifest 后自动提交 `plan`，选择全部 manifest 组件并固定 `allow_different`；`confirm_conflicts=true` 对应入口页及弹窗已说明的音频保留策略，`confirm_identity` 使用开始时用户勾选状态。SD 数据恢复到当前插入并挂载的 SD 卡；
...
| Config Web 页面：自动全量同设备备份、无密码、单页一键上传/校验/恢复、自动 plan、文字状态且无进度条及 Hide、刷新重绑、HTTP UUID/SHA-256 后备、会话续建、维护互斥、EN/ZH 文案 | 完成 | 本次修改的验证结果见测试记录；原有全链路验证需随运行实例更新重跑 |
