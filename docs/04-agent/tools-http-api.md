...
For `touch_gesture`, `back` swipes from near the left physical edge, and `home` swipes up from near the bottom physical edge; normalized coordinates use the 0-1000 range. When manually writing `swipe`, also use edge-aligned start points, e.g., `start.x=1` or `start.y=999`. Android defaults to touchscreen reports, but `{"type":"tap","point":{"x":500,"y":500},"input_device":"mouse"}` sends a real mouse click and makes Android display its pointer.
