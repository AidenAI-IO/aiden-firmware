...
Tool availability is not encoded as runtime context. It belongs to the tool catalog. For example, when iOS PiP Bridge is active while Aiden is backgrounded, `open_app` is omitted from resolved tools because PiP hides the Dynamic Island return path and app launching is not background-safe; background-safe data tools such as `clipboard`, `calendar`, `contacts`, and `notification` can remain exposed through the HTTP command queue.
