...
| Field         | Default | Description |
| ------------- | ------- | ----------- |
| `device_type` | `iOS`   | Target host type for USB HID descriptors and Agent global device state. Accepted values: `iOS`, `Android`, `macOS`, `windows`, `linux`. Android exposes both an absolute mouse and a touchscreen, with `pointer_mode = "touchscreen"` selecting touchscreen as the default gesture surface. Every other value derives `pointer_mode = "absolute"`. Changing it requires a reboot so USB descriptors are re-enumerated. |
...
| `mouse_device`            | `/dev/hidg1`                            | Absolute mouse HID device. On Android it produces real mouse events and a visible system pointer.                                                                                                                                                                                                                                                                                                                                                                                                                             |
