...
| `use_backend_agent` | realtime mode (`[voice_settings.realtime]`) | Realtime-mode switch, the single decision point for backend agent use (no model auto-detection): unset/`true` = legacy backend agent (regular realtime model); `false` = tools exposed directly to the session, omitting task/flow-control tools (`*_agent_task`, `request_user_action`, `wait_for_wakeup`). |
