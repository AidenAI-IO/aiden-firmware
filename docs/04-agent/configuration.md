...
| `use_backend_agent` | Gemini/OpenAI/Qwen/xAI (realtime) | Realtime-mode switch: unset inherits the model default (extended-thinking = direct tools, others = backend agent); `true` forces the legacy backend agent (regular realtime model); `false` exposes tools directly to the session and omits task/flow-control tools (`*_agent_task`, `request_user_action`, `wait_for_wakeup`). Settings on the provider record override the top-level value. |
