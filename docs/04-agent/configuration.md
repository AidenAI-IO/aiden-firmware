...
| `context_prune_threshold`   | `0`                         | Estimated token count that independently triggers deterministic cleanup of expired state snapshots and historical tool results. `0` derives the trigger from the active model input budget; a positive value is honored exactly. The cleanup target is 70/80 of the trigger to provide hysteresis. |
