...
Assistant responses begin with the concise spoken form in `<tts>...</tts>`, followed by the user-facing text, including responses that invoke tools. Only the first leading TTS block in each LLM response is streamed; when its closing `</tts>` tag arrives, the runtime immediately flushes the provider so short MiniMax speech does not wait for the remaining visible response. If the model omits the closing tag, the response boundary is treated as an implicit `</tts>` and flushes the remaining speech instead of dropping it. When a tool-call event arrives, the runtime resets the response-level parser state and skips the separate tool-event playback if that leading block already streamed, preventing duplicate speech while preserving the tool-event path as a fallback.
