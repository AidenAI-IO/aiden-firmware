...
Every assistant response uses one protocol: its first bytes are exactly one leading `<tts>...</tts>` block, followed by the user-facing text. This applies both to final answers and to progress content emitted with tool calls. For stories, jokes, readings, explanations, and other content intended to be heard in full, the complete spoken content belongs in that leading block rather than only an introduction or summary.
