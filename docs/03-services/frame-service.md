...
`frame_service` is a long-running C++ service that exclusively owns `/dev/video0`, continuously reads frames from the RK628D HDMI capture path, and provides the latest frames, screenshots, and health status externally through a Unix domain socket.
