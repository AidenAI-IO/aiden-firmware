...
The keyboard HID function is advertised as a report-protocol keyboard (`protocol=0`, `subclass=0`) instead of a boot keyboard. On iOS, the boot-keyboard path can intermittently accept plain keycodes while ignoring modifier chords; symptoms include `Shift+1` producing `1` instead of `!`, and `Cmd+A` / `Cmd+V` doing nothing. Force a clean composite gadget re-enumeration after descriptor changes before chasing HID timing or key mappings.
