...
If a connected phone has not finished ANCS subscription, `ble_service` retries
the existing discovery/subscription scan up to five times at two-second
intervals (plus D-Bus call time). Success or disconnect stops the retries;
authorization/permission/unsupported errors stop timed retries too. BlueZ
signals can still recover late discovery after the timer budget expires.
The status API is unchanged; only `ble_service` needs rebuilding and restarting.
For affected-phone acceptance, connect once, wait for `ancs_subscribed=true`,
and trigger a real test notification. If it remains false, capture the
`BLE ANCS retry` lines and errors in the BLE log before manually reconnecting.
