...
**核心理念**：尽量减少对用户的打扰，采用渐进式、多模态的提醒策略

1. **静默优先**：优先使用非语音方式（LED、Web UI）
2. **延迟提醒**：给用户完整的交互体验，不急于打断
3. **频率限制**：避免重复提醒造成的厌烦
4. **渐进升级**：从静默到语音，从温和到紧急
...
2. **Web UI 提示**
   - 在配置页面显示黄色横幅
   - "存储空间不足（剩余 XX MB），已自动清理部分数据"

3. **语音提醒**（仅在以下情况）
   - 第一次检测到时，**空闲超过 10 分钟**后提醒一次
   - 之后 24 小时内**不再语音提醒**（仅保持 LED + Web UI）
   - 如果用户访问 Web UI 或查看 LED，视为"已知晓"

**语音提醒时机**（如果触发）：
- 必须在**空闲状态**，且空闲超过 10 分钟
- 播放简短提示：`"提示：存储空间不足，已自动清理"`（< 5 秒）
- 不在用户交互过程中插入
...
2. **Web UI 提示**（立即）
   - 红色警告横幅
   - "存储空间严重不足（剩余 XX MB），部分功能已暂停"

3. **语音提醒**（延迟）
   - 第一次检测到：等待**空闲超过 3 分钟**后提醒
   - 之后每 **6 小时**最多提醒一次
   - 不在用户交互进行中打断
...
**语音文案**：
- `[紧急提示音] + "存储空间已满，无法使用。请立即清理存储空间"`

### 5.4 多模态提醒对比

| 级别 | LED | Web UI | 语音 | 频率 |
|------|-----|--------|------|------|
| 警告 | 🟡 慢闪 (5s/次) | 黄色横幅 | 10分钟空闲后 1次 | 24小时 1次 |
| 严重 | 🔴 中速闪 (2s/次) | 红色警告 | 3分钟空闲后 1次 | 6小时 1次 |
| 紧急 | 🔴 快闪 (0.5s/次) | 全屏遮罩 | 立即（有缓冲） | 每次唤醒 |
...
type AlertThrottle struct {
    lastAlertTime  map[string]time.Time  // 按级别记录
    alertCount     map[string]int        // 按级别计数
    userAckTime    time.Time             // 用户确认时间
}

// 提醒频率限制
- 警告级：首次 10分钟空闲后 1次，之后 24小时内不再语音提醒
- 严重级：首次 3分钟空闲后 1次，之后 6小时最多 1次
- 紧急级：每次用户交互都提醒（因为无法使用）

// 用户确认机制
- 如果用户访问 Web UI 存储页面，视为"已确认"
- 已确认后，降低语音提醒频率：
  - 警告级：不再语音提醒，仅保持 LED + Web UI
  - 严重级：延长到 12小时 1次
  - 紧急级：保持每次唤醒都提醒（必须清理才能用）
```
...
| 场景 | 原设计 | 优化后设计 | 改进 |
|------|--------|-----------|------|
| 警告首次检测 | 下次对话前插入 | 10分钟空闲后提醒 | ✅ 不打断交互 |
| 警告持续存在 | 每次对话都提醒 | 24小时仅 1次，优先 LED | ✅ 避免厌烦 |
| 严重播放中检测 | 播放完立即提醒 | 3分钟空闲后提醒 | ✅ 给用户缓冲 |
| 严重持续存在 | 不明确 | 6小时最多 1次 | ✅ 避免打扰 |
| 紧急播放中检测 | 立即打断全部 | 等当前句子完再打断 | ✅ 更自然 |
| 紧急录音中检测 | 立即停止 | 等自然停顿（最多2秒） | ✅ 更礼貌 |
...
```toml
[storage.alert]
# 语音提醒开关（LED 和 Web UI 始终启用）
voice_alert_enabled = true

# 警告级语音提醒（可选值：off, once, daily）
warning_voice_frequency = "once"  # off=关闭, once=仅一次, daily=每天一次

# 严重级语音提醒间隔（小时）
critical_voice_interval_hours = 6

# 紧急级立即打断（不可配置，必须为 true）
