...
def test_agent_exports_bridge_end_action_logs_when_artifact_dir_is_set(tmp_path):
    ...
    payload = json.loads((tmp_path / "trajectory" / "task" / "aiden_bridge_actions.json").read_text())
    assert payload[0]["action_id"] == "ep-artifact:0001"
    assert payload[0]["screenshot"] == {"format": "png", "data": "abc"}


def test_agent_exports_bridge_actions_under_task_trajectory_when_task_is_known(tmp_path):
    aiden = importlib.import_module("mobilegym.adapter.aiden_go_agent")
    ...
    agent.reset(type("Task", (), {"id": "account.WechatAccountCancellation"})())
    agent.act(obs=None)

    artifact = tmp_path / "trajectory" / "account_WechatAccountCancellation" / "aiden_bridge_actions.json"
    meta = tmp_path / "trajectory" / "account_WechatAccountCancellation" / "meta.json"
    assert json.loads(meta.read_text())["task_id"] == "account.WechatAccountCancellation"
    assert json.loads(artifact.read_text())[0]["tool_name"] == "tap"
