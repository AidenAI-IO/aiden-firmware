    ...
    awk -F '|' '
...

        {
            ...
            if (kind == "go") {
                ...
                if (source !~ /^https?:\/\//) {
                    fail("archive source for " name " must be http(s)")
                }
                if (length(source_sha256) != 64 || source_sha256 !~ /^[0-9A-Fa-f]+$/) {
                    fail("archive source_sha256 for " name " must be 64 hexadecimal characters")
                }
                if (artifact_path == "-" || artifact_path ~ /^\// || artifact_path ~ /(^|\/)\.\.(\/|$)/) {
                    fail("invalid archive artifact_path for " name ": " artifact_path)
                }
            }
...
rootfs_cli_catalog_names() {
    ...
    if [ "$strip_policy" = "all" ]; then
        ...
        printf '%s\n' "$records" | awk -F '|' -v strip_policy="$strip_policy" '$8 == strip_policy { print $1 }'
    fi
}
