...
def write_synthetic_dataset(
    ...
    t = np.arange(window_size, dtype=np.float32) / float(sample_rate)

    with dataset_path.open("w", encoding="utf-8") as dataset:
        for i in range(frame_count):
            if i % 4 == 0:
                audio = np.zeros((1, window_size), dtype=np.float32)
            elif i % 4 == 1:
                audio = rng.normal(0.0, 0.01, size=(1, window_size)).astype(np.float32)
            elif i % 4 == 2:
                ...
                audio = (
                    ...
                    + rng.normal(0.0, 0.015, size=window_size)
                )[None, :].astype(np.float32)

            audio = np.clip(audio, -1.0, 1.0).astype(np.float32)
            if externalize_stft:
                audio = silero_stft_magnitude(audio, window_size)
            elif model_input_size > window_size:
                padded_audio = np.zeros((1, model_input_size), dtype=np.float32)
                padded_audio[:, :window_size] = audio
                audio = padded_audio
...
def silero_stft_magnitude(audio: np.ndarray, window_size: int) -> np.ndarray:
    ...
    features = np.zeros((1, freq_bins, 3), dtype=np.float32)
...
def remove_silero_stft_frontend(model: onnx.ModelProto) -> tuple[int, int]:
    sqrt_index = None
    sqrt_output = ""
    for index, node in enumerate(model.graph.node):
    ...
    kept_nodes = [copy.deepcopy(node) for node in model.graph.node[sqrt_index + 1 :]]
    del model.graph.node[:]
...
def fix_silero_onnx_for_rknn(
    ...
    model_input_shape = [1, window_size]
    if not keep_stft_frontend:
        freq_bins, frames = remove_silero_stft_frontend(model)
    ...
    return str(fixed_path), [value.name for value in model.graph.input], int(np.prod(model_input_shape))
...
def main() -> int:
    ...
    if not args.generic_onnx:
        ...
        fixed_onnx, model_input_names, model_input_size = fix_silero_onnx_for_rknn(
            args.onnx,
            output_dir,
            args.window_size,
            args.state_size,
    ...
    try:
        ...
        print("--> Building quantized RKNN model for RV1106")
