...
if ! grep -q 'access("/userdata/wpa_supplicant.conf", F_OK)' "$RK_WIFI" || \
   ! grep -q 'access("/data/wpa_supplicant.conf", F_OK)' "$RK_WIFI"; then
    echo "rkwifi_server must prefer /userdata Wi-Fi config while preserving /data fallback" >&2
    exit 1
fi

userdata_line=$(grep -n 'access("/userdata/wpa_supplicant.conf", F_OK)' "$RK_WIFI" | sed 's/:.*//' | head -n 1)
...
if [ -z "$userdata_line" ] || [ -z "$data_line" ] || [ -z "$etc_line" ] || \
   [ "$userdata_line" -ge "$data_line" ] || [ "$data_line" -ge "$etc_line" ]; then
    echo "rkwifi_server must check /userdata before /data before /etc default" >&2
    exit 1
fi
...
if ! grep -q 'WPA_CONF=/etc/wpa_supplicant.conf' "$WIFI_INIT" || \
   ! grep -q 'WPA_CONF=/userdata/wpa_supplicant.conf' "$WIFI_INIT" || \
...
init_userdata_line=$(grep -n 'WPA_CONF=/userdata/wpa_supplicant.conf' "$WIFI_INIT" | sed 's/:.*//' | head -n 1)
...
if [ -z "$init_userdata_line" ] || [ -z "$init_data_line" ] || \
   [ "$init_userdata_line" -ge "$init_data_line" ] || \
   { [ -n "$init_cfg_line" ] && [ "$init_data_line" -ge "$init_cfg_line" ]; }; then
    echo "wifi init must prefer /userdata before /data before /data/cfg" >&2
    exit 1
fi
