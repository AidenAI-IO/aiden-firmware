...
std::string benchmark_suite_kind(const std::string& path) {
    FILE* suite_fp = fopen(path.c_str(), "r");
    if (!suite_fp) return "benchmark";
    ...
    if (sz > 0 && sz < 256 * 1024) {
        std::string buf(sz, '\0');
        size_t n = fread(&buf[0], 1, sz, suite_fp);
        buf.resize(n);
...
ApiResponse handle_request(const Options& options, const HttpRequest& request) {
    ...
    if (request.method == "GET" && request.path == "/benchmark/suites") {
        ...
        if (pipe) {
            ...
            while (fgets(line, sizeof(line), pipe)) {
                ...
                std::string kind = benchmark_suite_kind(path);
    ...
    if (request.method == "POST" && request.path == "/benchmark/run") {
        ...
        std::string suite_kind = run_suite_dir ? "unit" : benchmark_suite_kind(suite_path);
        // Write state.json
