...
void apply_kv(AgentToml& cfg,
    ...
    if (section == "model") {
        ...
        else if (key == "backend") assign_string(&cfg.audio.backend, raw, &sub_err);
        else if (key == "playback_backend" && cfg.audio.backend.empty()) assign_string(&cfg.audio.backend, raw, &sub_err);
        if (!sub_err.empty()) fail(sub_err);
...
bool save_agent_toml(const char* path, const AgentToml& input, std::string* error) {
    ...
    if (!cfg.audio.backend.empty()) emit_string(out, "backend", cfg.audio.backend);
    out << "\n";
