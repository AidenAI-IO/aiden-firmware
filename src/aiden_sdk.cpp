...
// 720p60 EDID for better device compatibility (Pixel 8, etc.)
static const uint8_t kDefaultHdmiEdid720p60[] = {
    ...
    0x55, 0x00, 0xf4, 0x19, 0x11, 0x00, 0x00, 0x1e,
    ...
    0x44, 0x36, 0x30, 0x0a, 0x20, 0x20, 0x01, 0x8c,
    0x02, 0x03, 0x0c, 0x00, 0x41, 0x84, 0x65, 0x03,
...
static_assert(sizeof(kDefaultHdmiEdid720p60) % 128 == 0,
...
static int push_edid(int subdev_fd, const CameraConfig& config) {
    ...
    if (config.edid_path) {
        // User provided custom EDID path
        if (load_edid_hex_file(config.edid_path, &heap_edid, &blocks) < 0) {
            return -1;
        }
        data = heap_edid;
    } else {
        // Choose default EDID based on requested resolution
        // Use 720p60 for 720p and below, 1080p30 for higher resolutions
        if (config.height <= 720) {
            data = kDefaultHdmiEdid720p60;
            blocks = sizeof(kDefaultHdmiEdid720p60) / 128;
        } else {
            data = kDefaultHdmiEdid1080p30;
            blocks = sizeof(kDefaultHdmiEdid1080p30) / 128;
        }
    }
