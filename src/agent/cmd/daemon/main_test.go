...
func TestRealtimeDelegatedToolSchemasMatchRuntimeTools(t *testing.T) {
	...
	definitions := make(map[string]rtclient.Tool)
	...
	for _, tool := range canonical {
		t.Run(tool.Name(), func(t *testing.T) {
			...
			if !bytes.Equal(definition.Function.Parameters, want) {
				t.Fatalf("realtime schema for %s = %s, want %s", tool.Name(), definition.Function.Parameters, want)
			}
		})
	}
}
