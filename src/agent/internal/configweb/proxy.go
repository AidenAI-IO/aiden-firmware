...
func (s *Server) proxyAgent(w http.ResponseWriter, r *http.Request, targetPath string) {
	...
	// The legacy portal normalized empty storage actions to an empty JSON
	// object before forwarding them. Keep that wire contract for clients that
	// POST /api/storage/eject or /api/storage/format without a body.
...
func (s *Server) doProxyAgent(r *http.Request, targetPath string, body []byte) (proxyResponse, error) {
	...
	requestTarget, err := url.Parse(targetPath)
	if err != nil {
		return proxyResponse{}, &proxyError{status: http.StatusBadRequest, message: "invalid request target"}
	}
	base.Path = strings.TrimRight(base.Path, "/") + requestTarget.Path
	base.RawQuery = requestTarget.RawQuery
...
func (s *Server) proxyAgentBytesRetry(w http.ResponseWriter, r *http.Request, targetPath string, body []byte, timeout time.Duration) int {
	...
	for {
		response, err := s.doProxyAgent(r, targetPath, body)
		if err == nil {
			// A completed HTTP exchange proves the restarted Agent is ready,
			// even when the endpoint returns a non-2xx application response.
