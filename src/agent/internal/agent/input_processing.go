...
func NewSTTClientFromConfig(cfg Config) (STTClient, error) {
	...
	case "openai", "openai-whisper":
		return NewOpenAIWhisperSTT(cfg.STT.APIKey, cfg.STT.Model, cfg.STT.BaseURL, newProxyHTTPClient(ProxyConfigFromEnvironment())), nil
	case "openrouter":
		return NewOpenRouterSTT(cfg.STT.APIKey, cfg.STT.Model, cfg.STT.BaseURL, newProxyHTTPClient(ProxyConfigFromEnvironment())), nil
	case "tencent":
