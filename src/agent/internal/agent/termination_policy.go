...
func (p *TerminationPolicy) AfterToolCall(toolName, input, observation string, isError bool) TerminationDecision {
	...
	if resultHash != "" && resultHash == previousResultHash && sameSig {
		p.sameResultStreak++
	} else {
...
func formatLoopGuardStopMessage(decision TerminationDecision, lastTool string) string {
	...
	parts := []string{
		"任务步骤过多且没有明显进展，我先停在这里。",
		fmt.Sprintf("停止原因：%s（%s）。", decision.Reason, decision.Message),
	}
	if lastTool != "" {
		parts = append(parts, fmt.Sprintf("最近一次工具：%s。", lastTool))
	}
	parts = append(parts, "请看一下当前屏幕，换个说法重试，或告诉我需要人工接手的地方。")
...
func observationProgressHash(toolName, observation string) string {
	...
	_, _ = h.Write([]byte{0})
	_, _ = h.Write([]byte(observation))
	return fmt.Sprintf("obs:%x", h.Sum64())
}
