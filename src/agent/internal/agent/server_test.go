...
func TestServerHandleChatReturnsToolHistory(t *testing.T) {
	...
	if len(resp.History) != 4 {
		t.Fatalf("expected 4 history entries for single-agent tool flow, got %d", len(resp.History))
	}
...
func TestServerHandleChatWithAudioAttachmentUsesSTT(t *testing.T) {
	...
	if len(resp.History) != 2 {
		t.Fatalf("expected 2 history entries for default-mode direct finish, got %d", len(resp.History))
	}
