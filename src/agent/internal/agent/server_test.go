...
func TestServerHandleHealthReportsEnvironmentBridgeReadiness(t *testing.T) {
	server := &Server{
		runtime: &Runtime{config: Config{Device: DeviceConfig{DeviceType: "Android"}}},
	}
	...
	if got.Data.Platform != "android" || got.Data.DeviceType != "Android" || got.Data.Concurrent != 1 {
		t.Fatalf("unexpected device health response: %#v", got.Data)
	}
...
func TestServerHandleReleaseReturnsNoOpSuccess(t *testing.T) {
	server := &Server{logger: newTestLogger()}
	...
	if !got.OK || !got.Data.Released {
		t.Fatalf("unexpected release response: %#v", got)
	}

	getReq := httptest.NewRequest(http.MethodGet, "/api/release", nil)
