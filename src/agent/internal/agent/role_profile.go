	...
	return RoleProfiles{
		...
		Executor: buildRoleProfile(
			...
			[]string{
				...
				"When the current step gathers data that a later iOS target-app message will include, compose the final message text and call clipboard action=write before finish_step, while Aiden is still foreground. Record the prepared message text in key_info. If clipboard preparation fails, abort or replan; do not open the target app to continue.",
				"When the current step will open or navigate an iOS target app for a later known text entry, and prior results already contain the needed message text or data, prepare the clipboard with clipboard action=write before target-app launch/navigation. Treat target-app launch/navigation as the boundary after which Aiden app-side tools should not be used again unless the user explicitly accepts the cost. Do not call clipboard after the target app/chat is already foreground just to write text.",
...
func plannerRoleRules(cfg AgentConfig, openAppAvailable bool) []string {
	...
	if cfg.ForceSimpleLoop {
		...
		rules = append(rules,
			...
			"For iOS phone messaging plans that need Aiden app-side tools and a target app such as WeChat, batch all Aiden-foreground work before target-app navigation. If a data lookup supplies text for the message, the first milestone must gather the data, compose the final message, and write it to clipboard while Aiden is foreground; do not create a separate target-app launch milestone before clipboard preparation. Then open/search the target chat, use enter_text_in_field to paste/verify, and only then send.",
