...
func plannerRoleRules(cfg AgentConfig, openAppAvailable bool) []string {
	...
	rules = append(rules,
		...
		"When target-app text entry will use previously prepared clipboard text, use enter_text_in_field or enter_text_via_bridge with the workflow artifact_id when one is available and send_after_commit=true only in the final message/form field. The runtime can resolve the prepared text from artifact_id.",
		"For message, email, or post send workflows, continue until current tool or screenshot evidence proves the send happened, such as an outgoing bubble, sent item, or cleared input after the send action.",
	)
	if openAppAvailable {
