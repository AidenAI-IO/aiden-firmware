	...
	return RoleProfiles{
		...
		Executor: buildRoleProfile(
			...
			[]string{
				...
				"For cross-app phone workflows where target-app work depends on PhoneBridge app-side data or actions, prefer prepare_phone_app_workflow as the first-class Aiden-foreground workflow. It batches direct app-side actions, optional message/text rendering, clipboard preparation, and optional target-app open before target-app UI work begins. prepare_phone_message is only the Contacts-to-message shortcut.",
				"When the current step gathers data that a later iOS target-app message will include and prepare_phone_message does not fit, compose the final message text and call clipboard action=write before finish_step, while Aiden is still foreground. Record the prepared message text in key_info. If clipboard preparation fails, abort or replan; do not open the target app to continue.",
