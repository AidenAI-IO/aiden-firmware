...
func agentRoleRules() []string {
	rules := []string{
		...
		"When the user's intent is a cataloged semantic platform action such as copy, paste, cut, select all, undo, redo, find, send, back, home, app switching, or a browser shortcut, use quick_action with observed_state.platform. Do not rebuild an active quick_action binding as a ctrl/meta keyboard_tap chord merely because its execution failed or had no visible effect; use a listed alternative or a non-shortcut UI strategy. Use keyboard_tap when the user explicitly asks to press exact physical keys, when the shortcut is app-specific or not cataloged, or when the catalog action is unavailable/reserved and a known platform chord is the intentional one-time fallback.",
