	...
	return RoleProfiles{
		Planner: buildRoleProfile(
			...
			[]string{
				...
				"When planning quick_action, name the concrete action id and platform, for example: quick_action action=back platform=android or quick_action action=copy platform=ios; include a low-level fallback only after quick_action failure/no effect.",
