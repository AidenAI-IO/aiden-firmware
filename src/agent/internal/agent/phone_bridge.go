...
func phoneBridgeTextEntryState(status PhoneBridgeStatus) string {
	...
	if phoneBridgeCanUsePiPBackground(status, "clipboard_write") || phoneBridgeCanUseFGSBackground(status, "clipboard_write") {
		return fmt.Sprintf("target_preserving_bridge: MUST call enter_text_via_bridge directly with platform=%q for non-search CJK/non-ASCII, multiline, or final composer text; do not call enter_text_in_field, do not type the target through IME, and do not stage bridge_clipboard manually; enter_text_via_bridge owns clipboard write, one shortcut attempt, verification, and long-press Paste fallback; if it returns committed=false but its attached screenshot appears to contain the target, observe once more before deciding, and never select_all, delete, or retype unless a fresh screenshot clearly proves the target is incomplete; set send_after_commit=true when the user asked to send/reply/message", platform)
	}
...
func phoneBridgeRuntimeContext(status PhoneBridgeStatus) string {
	...
	if phoneBridgeCanUseFGSBackground(status, "clipboard_write") {
		...
		builder.WriteString("- For non-search CJK/non-ASCII, multiline, or final composer text, call enter_text_via_bridge directly with platform=\"android\". Do not call enter_text_in_field or type the target through IME. It writes the clipboard without leaving the target app, tries the shortcut once, verifies it, then uses long-press Paste fallback; do not stage bridge_clipboard manually. If committed=false conflicts with an attached screenshot that appears complete, observe once more and preserve the existing text; never select all, delete, or retype unless a fresh screenshot clearly proves it is incomplete.\n")
		...
		builder.WriteString("- For non-search CJK/non-ASCII, multiline, or final composer text, call enter_text_via_bridge directly with platform=\"ios\". Do not call enter_text_in_field or type the target through IME. It writes the clipboard through the PiP queue, tries the shortcut once, verifies it, then uses long-press Paste fallback; do not stage bridge_clipboard manually. If committed=false conflicts with an attached screenshot that appears complete, observe once more and preserve the existing text; never select all, delete, or retype unless a fresh screenshot clearly proves it is incomplete.\n")
