...
func (t *TouchGestureTool) Description() string {
	return `Perform touch/pointer gestures. Provide either type or actions. Use the type form for normal interaction: tap, double_tap, long_press, swipe, drag_start, or drag_release. Atomic actions are a low-frequency advanced option only for an uninterrupted custom contact sequence that cannot be expressed by those standard gestures: {"actions":[{"action":"touch_down","point":{"x":500,"y":700}},{"action":"wait","ms":100},{"action":"move_to","point":{"x":500,"y":300},"speed":2500},{"action":"touch_up"}]}. Do not use actions for ordinary taps, long presses, swipes, scrolling, or moving draggable UI targets. Atomic actions execute in order and keep the contact pressed until touch_up. Atomic move_to accepts speed in normalized coordinate units per second; duration_ms overrides speed, and omitting both preserves immediate movement. ` +
		...
		`For swipe, provide start and either end or direction (up/down/left/right). Speed is average normalized coordinate units per second and defaults to 2500 for linear swipes; duration_ms may be supplied to override the calculated duration. profile defaults to linear. For list scrolling on HID or MobileGym, choose profile:decelerate, which starts moving promptly, slows continuously near the end, and releases without an added hold; it defaults to 600ms when both speed and duration_ms are omitted. Choose explicit start/end points near opposite INNER edges of the visible scrollable region, leaving some overlap; avoid fixed headers, bottom bars, and system gesture borders. Do not use arbitrary middle-screen endpoints to request a screenful. Shorten the distance near the target and inspect the returned screenshot after each swipe. This reduces release velocity but does not guarantee exact content displacement. Other providers may reject decelerate; use an explicit linear swipe with longer duration_ms there. hold_before_ms and hold_after_ms optionally dwell after press and before release, and steps controls HID interpolation (linear provider default 24; decelerate uses at least one sample per 16ms). Leave holds at zero for list scrolling. A direction-only swipe travels toward the corresponding screen edge when duration_ms is omitted, or travels speed*duration_ms/1000 normalized units when duration_ms is supplied. ` +
...
func (t *TouchGestureTool) ArgsSchema() map[string]any {
	typeDescription := "Gesture type. tap, double_tap, long_press, drag_start, and drag_release require point. swipe requires start and either end or direction."
	speedSchema := numberArgSchema("Average swipe speed in normalized coordinate units per second. Linear defaults to 2500; decelerate defaults to 600ms total when speed and duration_ms are omitted.", 2500)
	...
	schema := objectArgsSchema(map[string]any{
		...
		"end":            pointSchema(`End point for swipe. For list scrolling use explicit start/end near opposite inner edges of the visible scrollable region, with overlap. Swipe may provide direction instead of end.`),
		"profile":        stringEnumArgSchema("Swipe motion: linear (default), or decelerate for controlled list scrolling on HID/MobileGym. Decelerate slows before release without a hold; default duration is 600ms when speed and duration_ms are omitted.", "linear", "decelerate"),
