			...
			},
			{
				...
				Fields: []FieldMeta{
					...
					{Key: "responses_context_management", Widget: WidgetSelect,
						Help: "Responses API only. Let the provider compact stored context when the threshold is reached.",
						Enum: []EnumOption{
							{Value: "", Label: "provider compaction off"},
							{Value: "compaction", Label: "provider compaction"},
						},
						VisibleWhen: all(in("model.api_mode", "responses", "responses_stateful")),
						Default:     defaults.Model.ResponsesContextManagement},
					{Key: "responses_compact_threshold", Widget: WidgetNumber,
						Help:        "Optional token threshold for provider compaction; 0 uses the provider default.",
					...
					{Key: "responses_truncation", Widget: WidgetSelect,
						Help: "Choose whether the provider may truncate old input when supported.",
						Enum: []EnumOption{
					...
					{Key: "responses_include", Widget: WidgetList, Layout: "wide",
						Help:        "Optional Responses API include values, one per line (for example reasoning.encrypted_content).",
						VisibleWhen: all(in("model.api_mode", "responses", "responses_stateful")),
