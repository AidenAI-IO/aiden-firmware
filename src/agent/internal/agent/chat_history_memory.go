...
// Hot-window boundary markers. These are injected only at prompt-build time
// (never persisted into chat history) to tell the model where the compressed
// summaries end and the live hot window begins.
const (
	hotWindowStartMarker = "=== Recent session context (hot window) ===\n" +
	...
	hotWindowEndMarker = "=== End of recent context ===\n" +
...
// hotWindowBoundaryMemory brackets the rendered hot-window history with
// boundary markers when earlier history has been compressed. It wraps the
// window-buffer memory so the markers live only in the prompt string and never
// enter the persistable ChatMessageHistory.
type hotWindowBoundaryMemory struct {
	inner       schema.Memory
	compressedFn func() bool
}

func newHotWindowBoundaryMemory(inner schema.Memory, compressedFn func() bool) schema.Memory {
	if inner == nil || compressedFn == nil {
		return inner
	}
	return &hotWindowBoundaryMemory{inner: inner, compressedFn: compressedFn}
}

func (m *hotWindowBoundaryMemory) GetMemoryKey(ctx context.Context) string {
	return m.inner.GetMemoryKey(ctx)
}

func (m *hotWindowBoundaryMemory) MemoryVariables(ctx context.Context) []string {
	return m.inner.MemoryVariables(ctx)
}

func (m *hotWindowBoundaryMemory) LoadMemoryVariables(ctx context.Context, inputs map[string]any) (map[string]any, error) {
	...
	values[key] = hotWindowStartMarker + "\n\n" + existing + "\n\n" + hotWindowEndMarker
	return values, nil
}

func (m *hotWindowBoundaryMemory) SaveContext(ctx context.Context, inputs map[string]any, outputs map[string]any) error {
	return m.inner.SaveContext(ctx, inputs, outputs)
}

func (m *hotWindowBoundaryMemory) Clear(ctx context.Context) error {
