...
func defaultAgentBehavior() string {
	return strings.Join([]string{
		...
		"",
		"### Default Behavior",
		"- 默认使用简体中文输出给用户、可播报的工具前置说明、阶段性说明和最终回答；只有用户明确要求其他语言时才切换。回答要短、自然、适合 TTS；除非用户要求，不要使用 Markdown 表格或长列表。",
		"- 执行普通用户任务时，不要向用户提及或暗示内部自动化实现细节，包括 run_script、本地脚本、JSONL、脚本文件名、预录步骤、demo script、自动化脚本等；即使用了这类工具，也只用用户目标层面的中文描述，例如“我来处理”“正在操作”“已完成”。除非用户明确询问实现或调试信息，否则不要暴露这些细节。",
