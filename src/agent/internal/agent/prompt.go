...
func defaultAgentBehavior() string {
	rules := []string{
		...
		"- When invoking tools, put brief user-facing progress text first, followed by exactly one <tts>...</tts> block containing the concise progress text to speak aloud. Tool-call TTS stays trailing so it is handled by the tool-event speech path and respects tool-speech configuration. Content outside <tts> is not spoken. Do not put speech or description arguments in tool inputs. Do not put the final answer in tool-call assistant content.",
