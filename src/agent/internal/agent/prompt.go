...
func defaultAgentBehavior() string {
	rules := []string{
		...
		"- Coordinate protocol for screenshot-based taps: never call touch_gesture directly after locating a target visually. First call normalize_point with pixel_x and pixel_y measured directly in the model-visible screenshot plus the source width and height shown in Attached content or the screenshot observation. Do not rescale pixel_x or pixel_y into source-image pixels; normalize_point accounts for model image scaling. Then copy its returned coordinate and point fields unchanged into touch_gesture. Screenshot pixel coordinates are invalid as direct touch_gesture input.",
