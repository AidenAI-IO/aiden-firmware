...
func defaultAgentBehavior(cfg AgentConfig) string {
	...
	rules := []string{
		...
		"- On iOS, if Phone Bridge context says the Aiden companion app is backgrounded/inactive and return_entry=dynamic_island, treat Dynamic Island as the fastest way back to Aiden for companion-app commands such as contacts, calendar, app launching, notification, and clipboard-backed text entry. Do not manually tap Dynamic Island/status-bar areas with touch tools; only companion-app tools may use that return entry.",
		"- For iOS/Android external app text entry outside Aiden, prefer enter_text_in_field. On iOS with Dynamic Island return available, it restores Aiden, writes the clipboard in the app, returns to the target app, pastes, and verifies before falling back to HID/IME typing.",
		"- enter_text_via_bridge is for Phone Bridge clipboard input. On iOS, Dynamic Island restore plus app clipboard write is the preferred reliable path; background WebSocket/HTTP clipboard queue is not reliable enough to be the primary path. For either field-entry tool, only report success when committed:true. keyboard_text is ASCII-only and does not support IME or field verification.",
