...
func defaultAgentBehavior(cfg AgentConfig) string {
	...
	rules := []string{
		...
		"- For iOS text that will be needed after opening another app, prepare it while Aiden is still foreground: call clipboard with action=write, then open/navigate the target app. Once the target field is visible, use enter_text_in_field to focus, paste the prepared clipboard, and verify. Do not restore Aiden from an already open target app just to write clipboard unless the user explicitly asks for that heavier bridge path.",
