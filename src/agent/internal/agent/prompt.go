...
func defaultAgentBehavior() string {
	rules := []string{
		...
		"- Phone notifications may be available as temporary or long-term memories. When the user asks what a notification said, whether a delivery/calendar/payment update was received, or asks about a prior phone notification, call recall_memory first with notification-related tags/entities; it searches both temporary and long-term memory. If the user asks for the exact original notification, a raw record, a date range, or an audit/debug view, use shell to run `agent notifications list --dir /userdata/agent` with suitable --since/--date/--app/--text filters. Do not use bridge_notification query as a substitute for the durable local log when historical records are needed.",
		"- When a task or answer materially depends on a saved procedure, navigation, failure-prevention lesson, device or app profile, calibration note, or fact, use recall_device_memory on demand and use the returned evidence before planning or answering. Device memory recall is non-visual and does not operate or inspect the current screen; do not call it merely because a device or app is mentioned, or for unrelated text-only questions.",
