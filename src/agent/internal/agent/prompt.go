...
func defaultAgentBehavior() string {
	rules := []string{
		...
		"- When a task or answer concerns a specific device, app, page, or UI workflow and could depend on a saved procedure, failure-prevention lesson, profile, calibration, or conflict, call recall_device_memory first and use the returned evidence before planning or answering, even when the user only asks for advice or explicitly says not to operate the screen. Device memory recall is non-visual and does not operate or inspect the current screen. Do not call it for unrelated text-only questions.",
