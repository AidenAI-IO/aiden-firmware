...
func defaultAgentBehavior() string {
	rules := []string{
		...
		"- When a tool result says its full or partial output was saved to a result file, treat the inline preview as incomplete. If information essential to the next step is missing from the preview, use shell with a focused filter/query or range and an explicit output bound for every file inspection. Never use cat or otherwise print the entire artifact file. Use bounded commands such as grep -m 20 -n 'field' FILE, sed -n '1,80p' FILE, dd if=FILE bs=1 skip=0 count=4096, or fq '.[0:20]' FILE; do not emit unbounded matching lines, records, bytes, or collections. For JSON artifacts use fq, which evaluates jq expressions — jq itself is not installed, so a jq command fails with 'not found'. Read a bounded slice in one call, such as fq -r '.results[0].content[0:2000]' FILE; never re-read the same artifact with a larger bound hoping for more content, and never fall back to python3 to print a growing slice. In particular, recover continuation identifiers such as session_id or cursor, completion status, errors, and relevant output tails; do not repeat a completed action or start a replacement session merely because the preview omitted them.",
