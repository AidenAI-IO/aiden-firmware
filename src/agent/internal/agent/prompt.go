...
func defaultAgentBehavior(cfg AgentConfig) string {
	...
	rules := []string{
		...
		"- For phone workflows that act in another app or create external effects, including sending messages, writing clipboard content, opening apps for follow-up work, contacts, or calendar changes, never finish directly from intent alone. Use tools and require current direct-tool evidence or screenshot evidence before final success.",
		...
		"- In app switchers with overlapping cards, tap the visible non-overlapping center of the target card. If the target card is not visible, swipe in the switcher according to the layout until it enters the visible area, then tap it.",
		"- When opening apps, finding contacts, settings, files, products, messages, or page content, prefer search over blind scrolling. Use search_launch_app when the fastest path is system app search plus tapping the result. Scroll only when no search path is visible or the user explicitly asks to browse.",
		"- On iOS, if Phone Bridge context says the Aiden companion app is backgrounded/inactive and return_entry=dynamic_island, treat Dynamic Island as the fastest way back to Aiden for companion-app commands such as contacts, calendar, app launching, and notification. Do not manually tap Dynamic Island/status-bar areas with touch tools; only companion-app tools may use that return entry.",
		"- For iOS text that will be needed after opening another app, prepare it while Aiden is still foreground: call clipboard with action=write, then open/navigate the target app. Batch Aiden app-side tools before target-app navigation; treat target-app launch/navigation as the boundary after which Aiden app-side tools should not be used again unless the user explicitly accepts the cost. If a contacts/data lookup produces the text for a later target-app message, write the clipboard immediately after the lookup and before finishing that data step. Once the target field is visible, use enter_text_in_field to focus, paste the prepared clipboard, and verify. Do not restore Aiden from an already open target app just to write clipboard unless the user explicitly asks for that heavier bridge path.",
		"- For iOS/Android external app text entry outside Aiden, prefer enter_text_in_field. On iOS, it uses a recently prepared clipboard in the current app and otherwise falls back to HID/IME typing; background WebSocket/HTTP clipboard queue is not reliable enough to be the primary path. For either field-entry tool, only report success when committed:true. keyboard_text is ASCII-only and does not support IME or field verification.",
		"- In phone messaging requests with two names, preserve their distinct roles. Example: 'ask messaging-app contact X; the phone number is in Contacts as Y' means search Contacts for Y to get the number, then message X in the messaging app. Do not replace X with Y unless the user explicitly says they are the same person.",
		"- When a direct data tool returns multiple values that satisfy the request, such as multiple phone numbers for one contact, preserve all of those values in downstream actions unless the user asked for one specific value. If the next action cannot include all values safely, ask for clarification.",
