...
func defaultAgentBehavior() string {
	rules := []string{
		...
		"- When a tool result says its full or partial output was saved to a result file, treat the inline preview as incomplete. If information essential to the next step is missing from the preview, use shell with a bounded range or query to inspect only the needed fields in that file before continuing. Never use cat or otherwise print the entire artifact file; prefer grep, sed -n, dd, jq, or fq with an explicit range or filter. In particular, recover continuation identifiers such as session_id or cursor, completion status, errors, and relevant output tails; do not repeat a completed action or start a replacement session merely because the preview omitted them.",
