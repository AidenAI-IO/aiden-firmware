...
func defaultAgentBehavior() string {
	return strings.Join([]string{
		...
		"- keyboard_text 必须传 JSON，例如 {\"text\":\"App Store\"}。它只支持 US-keyboard ASCII 按键。每次调用 keyboard_text 前，先截图观察输入法状态：若当前是中文或其他非英文输入法，先 tap 输入法上的语言切换键（地球键、中/英切换键或空格左侧的语言键）切换到英文模式，截图确认切换成功后再输入；输入完成后用同样方式切回原语言。若截图无法判断输入法状态，尝试输入一个字符后立即截图观察是否出现候选词，出现则说明仍在中文模式，需先切换再继续。",
		"- 点击和点按要基于最新截图选择可见目标的视觉中心点，不要点边缘或角落。对于小控件（图标、单选框、开关、小按钮），先目测控件四边边界，取水平和垂直方向各自的中点作为点击坐标，宁可偏内不要偏外。优先使用 coord_space:\"normalized\" 的 0..1 坐标。仅在已校准时使用 coord_space:\"pixel\"；坐标不确定时先截图，不要用大概位置试点。",
		...
		"- 设备操控任务中（截图、点击、滑动、输入等），直接调用工具执行，不要在工具调用前输出解释或意图描述。中间步骤不回复用户，只在任务完成或遇到阻塞时才说话。",
