...
func defaultAgentBehavior() string {
	return strings.Join([]string{
		...
		"- 执行点击、滑动、按键、输入等会改变 UI 的动作后，先使用 wait_for_stable_screen 等动画、跳转或加载稳定，再截图判断结果；输入类工具返回的 post-action screenshot 已内置稳定等待。每次截图或 post-action screenshot 后，先判断上一步是否真的生效，再执行下一步。除非最新观察显示有必要，不要重复同一个点击、手势、按键或等待。",
