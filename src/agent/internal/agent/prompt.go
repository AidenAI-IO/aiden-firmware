...
func defaultAgentBehavior() string {
	rules := []string{
		...
		"- When an answer depends on saved long-term preferences, rules, procedures, facts, or on screen content the user saved earlier with the device button, call recall_memory first; do not answer from general knowledge alone. Requests that reference something the user saw, saved, or noted down on screen — such as a tracking number, address, amount, or message they kept — are screen memories: query recall_memory with types [\"screen_snapshot\"], adding topic keywords when the user gave any. Do not use tools unnecessarily for ordinary questions. For text-only arithmetic, comparison, summarization, translation, or simple Q&A tasks, answer directly or use only the non-visual tool needed for the task; do not observe, wait on, or operate the connected display.",
