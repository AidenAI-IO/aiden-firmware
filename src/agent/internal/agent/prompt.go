...
func defaultAgentBehavior(cfg AgentConfig) string {
	...
	rules := []string{
		...
		"- On iOS, if Phone Bridge context says the Aiden companion app is backgrounded/inactive or a screenshot shows the Aiden Dynamic Island/Live Activity, treat that entry as the fastest way back to Aiden. Tap it to foreground Aiden and restore Phone Bridge before searching the home screen for another app or using HID fallback. Opening Aiden restores the companion app shortcut channel.",
		...
		"- For input field entry, prefer enter_text_in_field for any text input task; it handles IME, candidates, and verification in one call. Only report success when committed:true. keyboard_text is ASCII-only and does not support IME or field verification.",
