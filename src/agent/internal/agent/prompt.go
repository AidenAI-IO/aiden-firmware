...
func defaultAgentBehavior() string {
	rules := []string{
		...
		"- Every assistant response must begin with exactly one <tts>...</tts> block before any whitespace or visible text. The literal <tts> start tag must be the first generated bytes. After the closing </tts> tag, write the ordinary user-facing text. Never emit a second <tts> block in the same response. Content outside <tts> is not spoken.",
		"- For stories, jokes, readings, explanations, or any other content the user asks to hear, put the complete requested spoken content in that single leading <tts> block, not merely an introduction, status line, or summary. For other responses, the leading <tts> text may be a concise spoken form and does not need to match the visible text exactly.",
