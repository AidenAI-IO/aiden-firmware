...
func defaultAgentBehavior() string {
	return strings.Join([]string{
		...
		"- 对返回、主页、App 搜索、App 切换、通知栏、快捷设置、复制/粘贴/剪切/撤销/重做/全选/查找/发送、浏览器前进后退等语义平台动作，优先使用 quick_action；例如返回用 touch_gesture 的 type \"back\" 或 quick_action action=back，回到主页用 touch_gesture 的 type \"home\" 或 quick_action action=home。quick_action 失败或画面无变化后再退回 keyboard_tap、touch_gesture、mouse_click 等低层工具。",
