...
func agentEnvironmentGuidance() string {
	return strings.Join([]string{
		...
		"- Prefer the Python standard library and already installed packages for controller-side Python work. Check for a previously managed package with command-scoped PYTHONUSERBASE=$AIDEN_PYTHON_USERBASE and /usr/bin/python3 before deciding it is missing. If a dependency is missing and AIDEN_PYTHON_USERBASE and AIDEN_PYTHON_TMP are available, first avoid installation when /run/agent/storage_level reports a non-normal level or /userdata is visibly low on free space. Otherwise use shell to create those directories and run /usr/bin/python3 -m pip install for one exact top-level name==version with command-scoped PYTHONUSERBASE=$AIDEN_PYTHON_USERBASE, TMPDIR=$AIDEN_PYTHON_TMP, PIP_USER=1, PIP_NO_CACHE_DIR=1, and PIP_DISABLE_PIP_VERSION_CHECK=1; pass --only-binary=:all: and --no-cache-dir, then run /usr/bin/python3 -m pip check with the same command-scoped PYTHONUSERBASE and a shell timeout of at least 120 seconds. If pip check times out, retry it once with a longer bounded timeout; do not claim installation succeeded unless pip check completes successfully. Run Python code that needs the package with the same command-scoped PYTHONUSERBASE and /usr/bin/python3, and append $AIDEN_PYTHON_USERBASE/bin to PATH only for an invocation of a package CLI. Never self-upgrade the firmware-provided pip, setuptools, or wheel; inspect recoverable errors, avoid unbounded retries, retry only with a compatible exact version, and report native or system-library failures instead of installing system packages.",
