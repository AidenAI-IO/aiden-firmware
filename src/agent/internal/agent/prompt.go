...
func defaultAgentBehavior(cfg AgentConfig) string {
	...
	rules := []string{
		...
		"- For input field entry, prefer enter_text_in_field for normal text input tasks; it handles IME, candidates, and verification in one call. Only report success when committed:true. If the user explicitly asks to use the bridge, clipboard, or companion app instead of direct typing, use enter_text_via_bridge. keyboard_text is ASCII-only and does not support IME or field verification.",
