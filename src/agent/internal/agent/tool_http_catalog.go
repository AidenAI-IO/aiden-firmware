...
func buildHTTPToolSkillMarkdown(name, description string, baseURL string, descriptors []ToolDescriptor) string {
	...
	builder.WriteString("- Phone Bridge routing is state-dependent: foreground Aiden uses WebSocket; iOS background with a recently polling PiP Bridge queue can run only background-safe data tools (`bridge_clipboard`, `bridge_calendar`, `bridge_contacts`, `bridge_notification`) through the HTTP queue; `bridge_open_app` and UI actions still require foreground Aiden, Dynamic Island restore, or HID/screenshot fallback.\n")
