...
func buildHTTPToolSkillMarkdown(name, description string, baseURL string, descriptors []ToolDescriptor) string {
	...
	builder.WriteString("- For input field entry, prefer `enter_text_in_field` for text input tasks; it handles IME and verification. `keyboard_text` is ASCII-only without IME support.\n")
