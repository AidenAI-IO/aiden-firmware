...
func buildHTTPToolSkillMarkdown(name, description string, baseURL string, descriptors []ToolDescriptor) string {
	...
	builder.WriteString("- For input field entry use `enter_text_in_field` once (IME switch + type + candidates + verify). Only report success when committed:true. `keyboard_text` is ASCII-only.\n")
