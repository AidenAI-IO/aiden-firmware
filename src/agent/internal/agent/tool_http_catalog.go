...
func buildHTTPToolSkillMarkdown(name, description string, baseURL string, descriptors []ToolDescriptor) string {
	...
	builder.WriteString("- Phone Bridge routing is state-dependent: foreground Aiden uses WebSocket; iOS background with PiP Bridge mode enabled can run only background-safe data tools (`clipboard`, `calendar`, `contacts`, `notification`) through the HTTP queue; `open_app` and UI actions still require foreground Aiden, Dynamic Island restore, or HID/screenshot fallback.\n")
	builder.WriteString("- For input field entry, prefer `enter_text_in_field` for normal text input tasks; it handles IME and verification. If the user explicitly asks for bridge/clipboard/companion-app input, use `enter_text_via_bridge`. `keyboard_text` is ASCII-only without IME support.\n")
