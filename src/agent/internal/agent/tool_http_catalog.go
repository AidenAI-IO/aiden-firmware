...
func buildHTTPToolSkillMarkdown(name, description string, baseURL string, descriptors []ToolDescriptor) string {
	...
	builder.WriteString("- For iOS/Android external app text entry, use `enter_text_in_field`. On iOS, batch reorderable Aiden app-side work before target-app navigation, including `clipboard`, `calendar`, `contacts`, and `notification`. Treat `open_app(target)` as the expensive phase boundary after which Aiden app-side tools should not be used just to prepare data or text. For cross-app phone workflows where the target app depends on PhoneBridge data/actions, prefer `prepare_phone_app_workflow` instead of separate app-side tool + `clipboard` + `open_app` calls; it runs structured direct app-side actions first, prepares final `target_text` clipboard while Aiden is foreground when needed, and can open the target app last. Do not treat UI-only app reading as reorderable direct data work unless a native PhoneBridge tool exists for that data. If the text is known and no workflow tool fits, call `clipboard` with action=write before `open_app`/target navigation. Once the field is visible, `enter_text_in_field` focuses the field and verifies the returned or prepared target text; if clipboard paste is unavailable, HID/IME typing fallback is still valid. Text-entry verification is not send evidence: for message, email, or post workflows, perform an explicit send action after the field text is verified and confirm the send from tool or screenshot evidence. Do not rely on background WebSocket/HTTP clipboard delivery as the primary path. On Android, bridge clipboard/paste may work without leaving the target app, but HID/IME typing remains the fallback. `keyboard_text` is ASCII-only without IME support.\n")
