...
func buildHTTPToolSkillMarkdown(name, description string, baseURL string, descriptors []ToolDescriptor) string {
	...
	builder.WriteString("- For input field entry, prefer `enter_text_in_field` for search/contact input, normal IME entry, or when Phone Bridge is unavailable. When current Phone Bridge state provides a usable clipboard route, use `enter_text_via_bridge` for long, multiline, CJK, or other non-ASCII final composer text; it owns clipboard write, paste, and verification, so do not stage `bridge_clipboard` manually. `keyboard_text` is ASCII-only without IME support. If a Chinese IME is active but the target text is English, switch to the English/Latin keyboard first, commonly with the globe/input-method key.\n")
