...
func buildHTTPToolSkillMarkdown(name, description string, baseURL string, descriptors []ToolDescriptor) string {
	...
	builder.WriteString("- For iOS/Android external app text entry, use `enter_text_in_field`. On iOS, batch Aiden app-side work before target-app navigation: if Contacts/data lookup produces message text, write the final clipboard text before finishing that lookup step; otherwise, if the text is known while Aiden is foreground, call `clipboard` with action=write before `open_app`/target navigation. Treat `open_app(target)` as the phase boundary after which Aiden app-side tools should not be used just to prepare text. Once the field is visible, `enter_text_in_field` focuses, pastes the prepared clipboard, and verifies without reopening Aiden. Do not rely on background WebSocket/HTTP clipboard delivery as the primary path. On Android, it can use connected bridge clipboard/paste without leaving the target app. It falls back to HID/IME typing in the same call if clipboard input fails. `keyboard_text` is ASCII-only without IME support.\n")
