...
func buildHTTPToolSkillMarkdown(name, description string, baseURL string, descriptors []ToolDescriptor) string {
	...
	builder.WriteString("- Use `open_app` for semantic app-name launches. It selects foreground Phone Bridge or visible system search internally. Use `open_url` for HTTP or HTTPS webpages.\n")
