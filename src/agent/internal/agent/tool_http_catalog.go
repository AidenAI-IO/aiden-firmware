...
func buildHTTPToolSkillMarkdown(name, description string, baseURL string, descriptors []ToolDescriptor) string {
	...
	builder.WriteString("- The HTTP catalog stays complete for direct diagnostics. The conversational Agent receives a runtime-filtered subset: `open_app` stays available because it can fall back to visible system search; App-only tools are exposed only when foreground Phone Bridge, PiP/FGS polling, Dynamic Island restore, or iOS BLE Wake can execute them. `bridge_notification` may also remain available for direct BLE notification queries.\n")
