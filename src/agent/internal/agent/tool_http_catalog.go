...
func buildHTTPToolSkillMarkdown(name, description string, baseURL string, descriptors []ToolDescriptor) string {
	...
	builder.WriteString("- All model-facing pointer, touch, picker, and text-focus coordinates implicitly use normalized 0-1000 values where (0,0) is top-left, (1000,1000) is bottom-right, and (500,500) is center. Do not pass a `coord_space` field or raw screenshot pixels.\n")
