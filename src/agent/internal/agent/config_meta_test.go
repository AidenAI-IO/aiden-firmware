...
func TestConfigMeta_PreservesExistingFormPresentation(t *testing.T) {
	...
	want := map[string]displayMeta{
		...
		"model.reasoning_effort":                      {help: "Empty = auto. Options follow the selected model capability; off is shown only when the model supports disabling thinking."},
		"model.thinking_budget_tokens":                {label: "Thinking budget (tokens)", placeholder: "0 = auto", help: "Optional exact thinking budget. Shown for models that expose budget_tokens; 0 uses the model default or effort preset. Thinking tokens are drawn from max_response_tokens, so this must be at least 1024 and smaller than that limit."},
