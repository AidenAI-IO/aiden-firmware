...
// TestDetectRepeatedFailedToolCall_WithInterleaving covers the real-world
// pattern we saw in MobileGym SkillOpt runs: the planner alternates between
// the broken call and a benign probe (screenshot) but keeps coming back to
// the same broken call. Strict consecutive matching missed this; the
// windowed counter must catch it.
func TestDetectRepeatedFailedToolCall_WithInterleaving(t *testing.T) {
	openFail := roleExecutionResult{
		Action:    &schema.AgentAction{Tool: "open_app", ToolInput: `{"url":"https://www.ebay.com"}`},
		Step:      &schema.AgentStep{Action: schema.AgentAction{Tool: "open_app"}},
		ToolError: &ToolError{Code: CodeBridgeNotConnected, Message: "phone bridge not connected"},
	}
	screenshotOk := roleExecutionResult{
		Action: &schema.AgentAction{Tool: "screenshot", ToolInput: `{}`},
		Step:   &schema.AgentStep{Action: schema.AgentAction{Tool: "screenshot"}},
	}

	t.Run("open_app failures interleaved with screenshot still trip", func(t *testing.T) {
		executions := []roleExecutionResult{
			openFail,
			screenshotOk,
			openFail,
		...
		if !detectRepeatedFailedToolCall(executions) {
			t.Fatalf("3 identical open_app failures within window 8 must trip even with screenshots interleaved")
		}
	})

	t.Run("only two open_app failures interleaved does not trip", func(t *testing.T) {
		executions := []roleExecutionResult{
			openFail,
			screenshotOk,
			openFail,
		...
		if detectRepeatedFailedToolCall(executions) {
			t.Fatalf("only 2 matching failures must not trip (threshold is 3)")
		}
	})

	t.Run("open_app success in the middle resets", func(t *testing.T) {
		openOk := roleExecutionResult{
			Action: &schema.AgentAction{Tool: "open_app", ToolInput: `{"url":"https://www.ebay.com"}`},
			Step:   &schema.AgentStep{Action: schema.AgentAction{Tool: "open_app"}},
		}
		executions := []roleExecutionResult{
			openFail,
			openFail,
			openOk, // bug went away, counter resets
			openFail,
			openFail, // only 2 since reset
		}
		if detectRepeatedFailedToolCall(executions) {
			t.Fatalf("identical success between failures must reset the count")
		}
	})

	t.Run("failures outside the window do not count", func(t *testing.T) {
		other := roleExecutionResult{
			Action: &schema.AgentAction{Tool: "screenshot", ToolInput: `{}`},
			Step:   &schema.AgentStep{Action: schema.AgentAction{Tool: "screenshot"}},
		}
		executions := []roleExecutionResult{
		...
		if detectRepeatedFailedToolCall(executions) {
			t.Fatalf("failures before the trailing window must not count")
		}
	})
}
