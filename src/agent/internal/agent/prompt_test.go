...
func TestRolePromptsIncludeGlobalEnvironmentAndDeviceGuidance(t *testing.T) {
	...
	for _, profile := range []RoleProfile{profiles.Planner, profiles.Executor} {
		for _, want := range []string{
			...
			"never finish directly from intent alone",
			"suitable for TTS",
			"device-operator",
			"visible target UI",
			...
			"prefer enter_text_in_field",
			"prepare it while Aiden is still foreground",
			"Batch Aiden app-side tools before target-app navigation",
			"treat target-app launch/navigation as the boundary",
			"write the clipboard immediately after the lookup",
			"Do not restore Aiden from an already open target app",
			"background WebSocket/HTTP clipboard queue is not reliable enough",
			"preserve their distinct roles",
			"messaging-app contact X",
			"Contacts as Y",
