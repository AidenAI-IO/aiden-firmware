...
func (t *EnterTextViaBridgeTool) Description() string {
	return `Use the Phone Bridge clipboard path to place known text into an input field, then focus, paste, and verify. ` +
		...
		`Returns committed:true only when the exact target text is verified in the field; when send_after_commit=true, ok=true also requires send_verified:true. If committed:false conflicts with the attached screenshot and the target appears complete, call wait_for_stable_screen once before deciding. Preserve the existing text during this confirmation: do not select_all, delete, cut, or retype unless the fresh screenshot clearly proves the target is incomplete.`
}
