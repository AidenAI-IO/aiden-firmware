...
- For picker/wheel value changes, a visibly supported numeric keyboard/edit mode may be used first. Tap the selected center row at most once to open that mode, make one verified keyboard attempt, and inspect the returned screenshot. If it does not work, fall back to `wheel_nudge`. Use `wheel_nudge` for all direct wheel-row taps and drags; after a successful wheel nudge, runtime reserves that region so generic input cannot activate a field outside the picker.
...
Some picker implementations expose an explicit numeric keyboard/edit mode. It is acceptable to tap the selected center row once to open that mode and make one verified keyboard attempt. Confirm the exact requested value in the fresh post-action screenshot. If the keyboard does not appear, the input has no effect, the value is wrong, or the result cannot be confirmed, stop: Do not repeat blind keyboard input. Dismiss the keyboard if necessary and fall back to `wheel_nudge`. Direct taps on unselected picker rows and all picker drags still belong to `wheel_nudge`.
