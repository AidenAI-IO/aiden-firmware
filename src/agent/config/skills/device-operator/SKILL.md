...
- Use `wheel_nudge` for every picker/wheel interaction. It taps an adjacent visible target row when possible and otherwise performs a bounded drag. Never use `touch_gesture` to tap or drag a picker column; after a real wheel nudge, runtime reserves that column for `wheel_nudge` so generic input cannot activate a field outside the picker.
...
- Never extrapolate a target row beyond the rows actually visible in the latest screenshot. If the target is not visibly adjacent, call `wheel_nudge` again; do not guess a lower `y` coordinate because picker columns often have editable fields directly underneath them.
