...
- Treat `open_app` into the target app as a phase boundary: after the target app/chat is foreground, do not use Aiden app-side tools such as `clipboard` just to prepare text unless the user explicitly asks for that heavier bridge path. Do not manually scatter app switching, paste, and screenshot verification for one field entry; let `enter_text_in_field` run focus/paste/verify as one verified operation.
