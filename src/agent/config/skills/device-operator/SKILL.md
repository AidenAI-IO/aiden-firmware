...
All `wheel_nudge` geometry uses normalized 0-1000 coordinates. Convert pixel measurements from the latest screenshot into normalized `column_x`, `center_y`, `row_spacing`, and `visible_target_y`; do not pass a coordinate-space selector to `wheel_nudge`.
