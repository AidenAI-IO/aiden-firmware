...
- For picker/wheel value changes, prefer a visibly supported numeric keyboard/edit mode before `wheel_nudge`. Tap the selected center row at most once to open that mode, make one verified `keyboard_text` attempt, and inspect the returned screenshot. If it does not work, fall back to `wheel_nudge`; once wheel fallback begins for that picker, do not switch back to keyboard input. Use `wheel_nudge` for all direct wheel-row taps and drags; after a successful wheel nudge, runtime reserves that region so generic input cannot activate a field outside the picker.
