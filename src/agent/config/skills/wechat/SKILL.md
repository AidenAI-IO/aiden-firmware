...
metadata:
  ...
  allowed_tools: [open_app, screenshot, quick_action, touch_gesture, mouse_click, keyboard_tap, enter_text_in_field, enter_text_via_bridge]
---
...
- For iOS/Android chat message body input, use `enter_text_in_field` with the input box coordinates. On iOS, when Aiden is reachable via Dynamic Island, it restores Aiden, writes clipboard in the app, returns to WeChat, pastes, and verifies. On Android, it can use connected bridge clipboard/paste without leaving WeChat. It then falls back to HID/IME typing only if clipboard input fails.
