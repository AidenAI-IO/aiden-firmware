...
const messages = {
  'en-US': {
    ...
    'config.fields.agent.context_prune_threshold.help': 'Fraction of the usable model input budget that triggers cleanup of expired state and historical tool results, cleaning down to 6/7 of the trigger. Must be between 0 and 1; 0 uses 0.5. Capped at context_compaction_threshold so this cheap pass runs before the conversation summary.',
  ...
  'zh-CN': {
    ...
    'config.fields.agent.context_prune_threshold.help': '上下文 token 数达到模型可用输入预算的该比例时，清理过期 state 和历史工具结果，并清理至触发值的 6/7。取值需在 0 到 1 之间，设为 0 时使用 0.5。该值不会超过 context_compaction_threshold，以保证这个低成本清理先于对话压缩执行。',
