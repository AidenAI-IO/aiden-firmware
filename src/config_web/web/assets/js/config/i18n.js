...
const messages = {
  'en-US': {
    ...
    'config.fields.model.reasoning_effort.help': 'Empty = auto. Options follow the selected model capability; off is shown only when the model supports disabling thinking.',
    'config.fields.model.thinking_budget_tokens.label': 'Thinking budget (tokens)',
    'config.fields.model.thinking_budget_tokens.help': 'Optional exact thinking budget for budget-based models. 0 uses the model default or effort preset. Thinking tokens count against Max response tokens, so this must be at least 1,024 and smaller than that limit.',
    'config.fields.model.thinking_budget_tokens.placeholder': '0 = auto',
  ...
  'zh-CN': {
    ...
    'config.fields.model.reasoning_effort.help': '留空 = 自动。选项根据当前模型能力动态提供；仅支持关闭思考的模型显示 off。',
    'config.fields.model.thinking_budget_tokens.label': '思考预算（token）',
    'config.fields.model.thinking_budget_tokens.help': '仅预算型模型可用的精确思考预算。0 表示使用模型默认值或档位预设。思考 token 计入「最大响应 token」，因此该值需不小于 1024 且小于该上限。',
    'config.fields.model.thinking_budget_tokens.placeholder': '0 = 自动',
