...
function setBanner(message,isError){const el=byId('actionBanner');el.textContent=message;el.className='banner'+(isError?' error':'');}
    ...
    function agentURL(path){const current=new URL(window.location.href);const target=new URL(path,current.href);current.port='8080';current.pathname=target.pathname;current.search=target.search;current.hash=target.hash;return current.toString();}
    async function agentRequest(path,options){const res=await fetch(agentURL(path),options);const text=await res.text();let body={};try{body=text?JSON.parse(text):{}}catch(err){body={ok:false,error:text||err.message}}if(!res.ok){const error=new Error(body.error||('HTTP '+res.status));error.status=res.status;throw error;}return body;}
