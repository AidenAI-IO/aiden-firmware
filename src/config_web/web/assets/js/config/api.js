...
function setBanner(message,isError){const el=byId('actionBanner');el.textContent=message;el.className='banner'+(isError?' error':'');}
    ...
    // Runtime capabilities are served by the Agent on port 8080. Keep the
    // host and protocol of the page so USB-ECM and local development both use
    // the same device, while preserving the requested path and query.
    function agentURL(path){const current=new URL(window.location.href);const target=new URL(path,current.href);target.protocol=current.protocol;target.hostname=current.hostname;target.port='8080';target.username='';target.password='';return target.toString();}
    async function agentRequest(path,options){return request(agentURL(path),options);}
