...
const slowBackground = deferred();
let refreshRequestCount = 0;
requestImpl = () => {
  refreshRequestCount++;
  return slowBackground.promise;
};
const slowBackgroundRefresh = logsModule.namespace.refreshAgentLog(false);
await logsModule.namespace.refreshAgentLog(false);
assert.equal(refreshRequestCount, 1);
slowBackground.resolve({agent_log: {exists: true, log: 'slow background body', size_bytes: 20}});
await slowBackgroundRefresh;
assert.equal(agentLogText.children.at(-1).textContent, 'slow background body');
assert.equal(refreshLogButton.disabled, false);

const supersededBackgroundSuccess = deferred();
const manualAfterBackground = deferred();
const backgroundManualRequests = [supersededBackgroundSuccess, manualAfterBackground];
let backgroundManualRequestCount = 0;
requestImpl = () => {
  backgroundManualRequestCount++;
  return backgroundManualRequests.shift().promise;
};
const supersededBackgroundRefresh = logsModule.namespace.refreshAgentLog(false);
const manualAfterBackgroundRefresh = logsModule.namespace.refreshAgentLog(true);
await logsModule.namespace.refreshAgentLog(false);
assert.equal(backgroundManualRequestCount, 2);
manualAfterBackground.resolve({agent_log: {exists: true, log: 'manual winner body', size_bytes: 18}});
await manualAfterBackgroundRefresh;
assert.equal(agentLogText.children.at(-1).textContent, 'manual winner body');
assert.equal(refreshLogButton.disabled, false);
supersededBackgroundSuccess.resolve({agent_log: {exists: true, log: 'obsolete background success', size_bytes: 27}});
await supersededBackgroundRefresh;
assert.equal(agentLogText.children.at(-1).textContent, 'manual winner body');

const supersededBackgroundFailure = deferred();
const secondManualWinner = deferred();
const failureManualRequests = [supersededBackgroundFailure, secondManualWinner];
requestImpl = () => failureManualRequests.shift().promise;
const supersededBackgroundFailureRefresh = logsModule.namespace.refreshAgentLog(false);
const secondManualWinnerRefresh = logsModule.namespace.refreshAgentLog(true);
secondManualWinner.resolve({agent_log: {exists: true, log: 'second manual winner', size_bytes: 20}});
await secondManualWinnerRefresh;
supersededBackgroundFailure.reject(new Error('obsolete background failure'));
await supersededBackgroundFailureRefresh;
assert.equal(agentLogText.children.at(-1).textContent, 'second manual winner');
assert.doesNotMatch(agentLogMeta.textContent, /obsolete background failure/);

const olderManual = deferred();
const newerManual = deferred();
const manualRequests = [olderManual, newerManual];
requestImpl = () => manualRequests.shift().promise;
const olderManualRefresh = logsModule.namespace.refreshAgentLog(true);
const newerManualRefresh = logsModule.namespace.refreshAgentLog(true);
olderManual.resolve({agent_log: {exists: true, log: 'obsolete manual body', size_bytes: 20}});
await olderManualRefresh;
assert.equal(refreshLogButton.disabled, true);
newerManual.resolve({agent_log: {exists: true, log: 'newer manual body', size_bytes: 17}});
await newerManualRefresh;
assert.equal(refreshLogButton.disabled, false);
assert.equal(agentLogText.children.at(-1).textContent, 'newer manual body');
